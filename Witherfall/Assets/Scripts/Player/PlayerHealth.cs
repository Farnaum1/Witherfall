...
public class PlayerHealth : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        Spinner spinner = collision.GetComponent<Spinner>();

        if (spinner && !isInvincible)
        {
            TakeDamage(spinner.damage);
            Rigidbody2D rb = GetComponent<Rigidbody2D>();

            if (rb != null)
            {
                Vector2 pushDir = (transform.position - spinner.transform.position).normalized;
                rb.AddForce(pushDir * spinner.pushForce, ForceMode2D.Impulse);
            }
        }
    ...
    IEnumerator DeathSequence()
    {
        ...
        GameController.Instance.progressAmount = 0;
